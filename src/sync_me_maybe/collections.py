...
class CollectionResolver:
    ...
    def _expand_sync(self, classified: ClassifiedLink) -> list[TrackSearchItem]:
        ...
        if classified.kind == LinkKind.YOUTUBE:
            ...
            raise CollectionResolveError("This provider does not support playlist or album expansion.")
        ...
        if len(tracks) > self.settings.max_collection_tracks:
            raise CollectionResolveError(
                f"Collection has {len(tracks)} tracks, above MAX_COLLECTION_TRACKS={self.settings.max_collection_tracks}."
            )
        return tracks
    ...
    def _spotify_collection(self, classified: ClassifiedLink) -> list[TrackSearchItem]:
        ...
        if not tracks:
            raise CollectionResolveError(
                "Could not expand this Spotify collection. Public extraction did not expose track data."
            )
        return tracks
    ...
    def _apple_music_collection(self, classified: ClassifiedLink) -> list[TrackSearchItem]:
        ...
        if not tracks:
            raise CollectionResolveError(
                "Could not expand this Apple Music collection. Public extraction did not expose track data."
            )
        return tracks
...
def _tracks_from_entries(entries: list[Any]) -> list[TrackSearchItem]:
    ...
    for index, entry in enumerate(entries, start=1):
        ...
        tracks.append(
            TrackSearchItem(
                title=title,
                artist=clean_title(entry.get("artist") or entry.get("uploader") or entry.get("creator")),
...
def _track_from_dict(value: dict[str, Any]) -> TrackSearchItem | None:
    ...
    track_number = _int(source.get("trackNumber") or source.get("track_number") or source.get("position"))
    external_urls = source.get("external_urls") if isinstance(source.get("external_urls"), dict) else {}
    url = source.get("url") or external_urls.get("spotify")

    if not title:
        return None
    if not artist and "musicrecording" not in type_value and "track" not in type_value and not track_number:
        return None
    return TrackSearchItem(title=title, artist=artist, album=album, track_number=track_number, source_url=url)
