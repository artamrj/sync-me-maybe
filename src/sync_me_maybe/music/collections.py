...
from sync_me_maybe.music.providers.base import ExpandedCollection, ProviderError
...
class CollectionResolver:
    ...
    async def expand(self, classified: ClassifiedLink) -> ExpandedCollection:
        ...
        try:
            collection = await provider.expand_collection(classified)
        ...
        if len(collection.tracks) > self.settings.max_collection_tracks:
            raise CollectionResolveError(
                f"Collection has {len(collection.tracks)} tracks, "
